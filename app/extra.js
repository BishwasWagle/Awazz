...
function translate(source, destination, html, smart) {
    ...
    if (smart) {
        for (mask in nonSmartTokens) { unicode = unicode.replace(translateWords(mask, html).replace(" ", ""), nonSmartTokens[mask].replace(/\s$/, "")); }
    }
    for (mask in engTokens) { unicode = unicode.replace(translateWords(mask, html).replace(" ", ""), engTokens[mask]); }
...
function selectAll(txtbox) {
    ...
    if (txtbox.setSelectionRange) /*Mozilla*/ { txtbox.setSelectionRange(0, txtbox.value.length); } else if (txtbox.createTextRange) /*Opera 8*/ {
...
function convertLive(flag) {
    if (flag) {
        if (Form1.ascii.value.length > 1500) {
            alert("The text is too large for live conversion. Please press the 'Convert' button.");
            Form1.convertItLive[1].checked = true;
            return convertLive(false);
        }
        // Form1.convertNow.disabled = true;
        translate(Form1.ascii, Form1.unicode, eval(Form1.htmlEncode.value));
        liveConversion = true;
    } else {
...
function convert() {
    if (liveConversion == true && Form1.ascii.value.length <= 1500) { translate(Form1.ascii, Form1.unicode, eval(Form1.htmlEncode.value)); } else if (liveConversion == true && Form1.ascii.value.length > 1500) convertLive(true);
}
